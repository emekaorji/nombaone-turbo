...
/**
 * Persist the operator's mode PREFERENCE (test|live) and revalidate the
 ...
 */
export async function setEnvironment(mode: Mode): Promise<void> {
  ...
  store.set(ENV_COOKIE, mode, envCookieOptions());
