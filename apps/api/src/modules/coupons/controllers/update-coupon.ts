...
import { updateCoupon } from '@nombaone/sara/coupons';
...
/** PATCH /v1/coupons/:reference — redeemBy / maxRedemptions / metadata only. */
export const updateCouponController: RequestHandler = jsonHandler<CouponResponseData>(async (req) => {
  ...
  const data = await updateCoupon(db, ctx, req.params.reference ?? '', {
