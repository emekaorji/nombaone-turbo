...
/**
 * Active sandbox/live mode switch. Flipping it calls the `switchEnvironment` server
 ...
 */
export function EnvSwitcher({ mode }: { mode: Mode }) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();

  const onChange = (next: string) => {
    const env = next as Mode;
    if (env === mode) return;
    startTransition(async () => {
      try {
        await switchEnvironment(env);
        router.refresh();
        toast.success(`Switched to ${env === 'live' ? 'Live' : 'Sandbox'} mode`);
      } catch {
  ...
  return (
    <Select value={mode} onValueChange={onChange} disabled={pending}>
      <SelectTrigger
        className={cn(
