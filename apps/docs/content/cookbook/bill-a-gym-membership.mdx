...
description: "Iron Republic gym, end to end: connect your Nomba account, one plan with three prices, members joining through hosted checkout, silent card renewals with an honest OTP fallback, transfer memberships on the invoice lane, and dunning that recovers before it churns."
section: cookbook
order: 1
---
...
<Callout type="note" title="When there is no checkoutLink">
