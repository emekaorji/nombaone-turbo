...
description: The sandbox sends no organic webhooks, so we hand you a real signed event — register an endpoint, fire it, and watch the signature verify in your own logs.
...
<Callout type="warning" title="Verify the raw bytes, before parsing">
...
<CardGroup cols={2}>
  <Card title="Handle webhooks (guide)" href="/guides/handle-webhooks">
