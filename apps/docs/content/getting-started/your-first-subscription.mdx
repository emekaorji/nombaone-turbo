...
description: Create a plan, a price, a customer, and a subscription that bills — five real calls to a live 201, end to end.
...
<Callout type="tip" title="Get a test key first">
...
<Steps>
  ...
  <Step title="Add a price">
    ...
    <Callout type="warning" title="Money is integer kobo — the 100× trap">
  ...
  <Step title="Attach a test payment method that succeeds">
    ...
    `test_success` here; the other behaviors (decline, OTP required, thin
    ...
    <CodeGroup>
      <Tab label="cURL">
        ...
        curl -X POST https://sandbox.api.nombaone.xyz/v1/test/payment-methods \
          -H "Authorization: Bearer nbo_test_…" \
          -H "Content-Type: application/json" \
          -d '{ "customerId": "{customerId}", "behavior": "test_success" }'
        ```
      </Tab>
      <Tab label="TypeScript">
        ```ts
        const method = await nbo("/v1/test/payment-methods", {
          method: "POST",
          body: { customerId: customer.data.id, behavior: "test_success" },
        });
    ...
    <Callout type="note" title="Test-environment only">
  ...
  <Step title="Start the subscription">
    ...
    the method is `test_success`, it comes back **active** with a paid invoice.
...
<CardGroup cols={2}>
  <Card title="When the charge fails" href="/concepts/hard-parts/dunning-for-thin-balances">
    Thin balances mean a failed charge is usually "not yet," not "no." Dunning
    owns recovery.
  </Card>
  <Card title="Cards need OTP" href="/concepts/hard-parts/card-tokens-expire">
    A recurring card charge in Nigeria often hits a bank OTP step. See how the
    engine recovers with a fresh checkout link.
  </Card>
  <Card title="Pick a rail" href="/concepts/multi-rail-push-and-pull">
...
<Callout type="tip" title="Trials, proration, and idempotency">
