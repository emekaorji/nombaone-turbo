...
description: "Drive the engine deterministically on the sandbox: deterministic payment outcomes, a sandbox clock to skip the wait, and on-demand signed webhooks. No real card, no cron."
...
<Callout type="note" title="Sandbox mode only, live behavior identical">
...
<CardGroup cols={2}>
  ...
  <Card title="The sandbox clock" href="/sandbox-toolkit/clock">
