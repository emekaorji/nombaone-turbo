...
description: Attach a deterministic payment method on the sandbox — its behavior decides the outcome, so you can rehearse success, decline, and card-OTP paths on demand.
...
<Callout type="note" title="requires_otp is the one to rehearse most">
