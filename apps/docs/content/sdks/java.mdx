---
title: Java SDK
description: "One thread-safe, synchronous SDK for Java, Kotlin, and Spring — integer-kobo money, unchecked typed errors, automatic retries that never double-charge, auto-paginating lists, and a sealed webhook event union, with Jackson as its only dependency."
section: sdks
---
...
| `onResponse(Consumer<ResponseInfo>)` | Receive the raw `ResponseInfo` — status, request id, headers — on success. |
...
`autoPager()` is an `Iterable<T>` and `stream()` is a `Stream<T>`; both fetch pages
...
<Callout type="warning" title="The method is voidInvoice, because void is a Java keyword">
...
<CardGroup cols={3}>
  <Card title="Method reference" href="/sdks/java/reference">
