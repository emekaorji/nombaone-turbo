...
description: "The @nombaone/node SDK: one TypeScript-first library for Node, Next.js, and every JS runtime. Typed errors, automatic retries that never double-charge, cursor pagination, and a fully-typed webhook event union."
section: sdks
---
...
Every non-list method returns an `APIPromise<T>` — `await` it for the resource, or
call `.withResponse()` for `{ data, requestId, response }` when you need the raw
`Response` (headers, status, rate-limit info).

Every `list()` returns a `PagePromise<Item>`. Await it for one `Page`, or `for
...
<CardGroup cols={3}>
  <Card title="Method reference" href="/sdks/node/reference">
