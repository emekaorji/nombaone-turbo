...
description: Delivery is at-least-once, never exactly-once. The same event can arrive twice — dedupe on the event id and make every handler idempotent.
section: webhooks
order: 4
---
...
<Callout type="warning" title="Dedupe on the event id, after verifying">
...
<CardGroup cols={2}>
  ...
  <Card title="Retrying ≠ re-charging" href="/concepts/hard-parts/retry-the-webhook-is-not-retry-the-charge">
