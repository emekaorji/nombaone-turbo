---
title: Event catalog
description: Every event nombaone can emit, when it fires, and its payload — rendered directly from the API's canonical catalog, so it can never drift.
section: webhooks
order: 2
toc: false
---
...
<Callout type="note" title="One reference, everywhere">
