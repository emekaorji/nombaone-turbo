...
>
> The event you fire below is a genuine signed payload from the test instrument,
...
> sandbox delivery — it is test-deployment-only and at-least-once (dedupe on the
> event id).
...
Ask the test instrument to emit a signed event to your registered endpoint. Fire
