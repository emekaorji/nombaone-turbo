 ...
 * chevron opens a rich menu to view the Markdown, open the page pre-loaded in an
 * assistant (ChatGPT / Claude / Perplexity), copy the docs MCP server URL, or
 * one-click install that MCP server into Cursor / VS Code.
 *
 * The Markdown copied is the exact `.md` mirror the build emits, so an agent
 * gets the same bytes a human reads — no drift.
 */

const BASE = "https://docs.nombaone.xyz";
const MCP_URL = `${BASE}/api/mcp`;
const MCP_NAME = "nombaone-docs";

/** Base64 (browser) of the MCP client config for the one-click editor installs. */
function mcpConfigB64(): string {
  return typeof window === "undefined" ? "" : window.btoa(JSON.stringify({ url: MCP_URL }));
}
...
export function CopyPage({ slug }: CopyPageProps) {
  ...
  return (
    <div className="not-prose inline-flex overflow-hidden rounded-lg border border-border bg-card text-sm shadow-sm">
      <button
        type="button"
...
const ROW_CLASS =
  "flex w-full cursor-pointer items-center gap-2.5 rounded-lg px-2 py-2.5 text-left focus:bg-muted data-[highlighted]:bg-muted";
  ...
  const inner = (
    <>
      ...
      <span className="flex min-w-0 items-center gap-1 font-medium text-foreground">
        {row.title}
        {row.external && <ArrowUpRight size={13} aria-hidden className="opacity-45" />}
      </span>
    </>
  );

  return (
    <DropdownMenuItem asChild className={ROW_CLASS} onSelect={row.onSelect}>
      {row.href ? (
...
// ---------------------------------------------------------------------------
// Icons — monochrome (currentColor), sized for the 36px tiles.
// ---------------------------------------------------------------------------
...
function MarkdownIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 208 128" aria-hidden>
      <rect x="5" y="5" width="198" height="118" rx="12" fill="none" stroke="currentColor" strokeWidth="11" />
      <path
        fill="currentColor"
        d="M30 98V30h20l20 25 20-25h20v68H90V59L70 84 50 59v39H30zm132 0-30-33h20V30h20v35h20l-30 33z"
      />
    </svg>
  );
}
