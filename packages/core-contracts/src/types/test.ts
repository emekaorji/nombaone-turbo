 ...
 * Returned only by the `/v1/test/*` endpoints on a test deployment. Test payment
 * methods reuse `PaymentMethodResponseData` (a real, chargeable method row).
 */

/** Result of forcing one billing cycle via POST /v1/test/subscriptions/{id}/advance-cycle. */
...
/** Result of POST /v1/test/webhooks/simulate — the minted event + how many deliveries fired. */
