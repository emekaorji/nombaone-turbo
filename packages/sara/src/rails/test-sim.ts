 ...
 * A test payment method (seeded by the `/v1/test/payment-methods` helper) stores
 ...
 *
 * This is the ONLY place we branch a money path on environment. It is safe
 * because it returns `null` (⇒ fall through to the real rail) whenever the
 * environment is not `test` OR the method carries no sentinel, so live behavior
...
const TOKEN_RESULT: Readonly<Record<string, RailCollectResult>> = {
  ...
  test_requires_otp: {
    ...
    action: { type: 'otp_3ds', message: 'Simulated 3-D Secure / OTP step-up (test mode).' },
  },
};
