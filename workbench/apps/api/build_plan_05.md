...
- [ ] **C4** — interval-switch (monthly→annual) prorates correctly: unit + e2e assert old-cadence credit + new-cadence prorated debit, exact kobo. **DEFERRED** — currently REJECTED with `PRORATION_INTERVAL_SWITCH_UNSUPPORTED` (e2e proves the rejection), not mis-charged; correct interval-aware proration needs period-claim re-anchoring — its own reviewed build.
