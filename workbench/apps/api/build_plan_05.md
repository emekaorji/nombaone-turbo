...
- [x] **J9** — void path produces correct ledger entries: invoice-void (03c/03e) + discount-removal (moves no money) + **credit-void NOW BUILT** (`voidCreditGrant`): posts a `reversal` for ONLY the unconsumed `remaining` (a partially-consumed grant's consumed part already left `customer_credit`, so reversing the original full posting would over-reverse), sets `remaining=0`+`voided_at`, idempotent (`isNull(voided_at)` CAS). `DELETE /v1/customers/:ref/credit/:grantRef`. e2e: partial-consume then void → balance goes to exactly 0 (30k consumed + 70k voided = the full 100k, no over-reversal); double-void is a no-op. Migration 0013.
