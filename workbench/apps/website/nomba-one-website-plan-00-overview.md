...
**The hero now speaks to all four at once.** Its rotating-audience headline ("Subscriptions for every {audience}.") cycles the final word, rendered in the emerald gradient, through developer, founder, merchant, decision-maker, and program manager, with a caption naming the audiences. Every visitor sees themselves named in the first frame; the depth-layering below then serves each in turn.
...
> **Design language v2 (canonical).** The reworked, user-approved home page sets the current bar and supersedes the design-system file wherever they conflict: a **bigger, more confident type scale** (hero display up to 96px); **whitespace, not dividers**, between sections (no hairline rules); and **no eyebrows** (title + deck only). Everything else in the design system still holds. Where a value here conflicts with the older file, this note wins.
...
- **One signature accent: electric emerald.** Used with restraint: primary CTA, links, focus rings, and the simulator's success/recovery moment. Never decorative. Semantic hues (success/warning/danger/info) appear only in status and product contexts. **Accent-gradient pattern:** the hero's dynamic (rotating) word is filled with a linear emerald gradient, `#7deabd → #0bdfa3 → #00c38b`. Reserve that gradient for the headline moment, not general text.
- **Typography: Geist Sans + Geist Mono** (Vercel's own, open-licensed), tight negative tracking on display sizes. **Type scale v2 (bigger, confident):** hero display 96px / 600 / letter-spacing ~-4.4 / line-height 1.03; section title (H2) 56px / 600 / ls ~-2.4 / lh 1.05; section deck 24px / 400 / muted; hero lede 27px; section body and paragraphs 20-24px; card titles 24-26px; card body 17-18px; list / trust / feature items 18px; large stat numerals ~76px; small / caption 15-16px; mono code 14, mono caption 12-13. The big type and generous space are a deliberate signal: they read as the calm confidence of a team that is sure of what it built, not as decoration.
- **Motion is tokenised** (durations + three easing curves). See doc 03.
- **No eyebrows.** A section header is a **title + one-line deck** only. There is no mono-uppercase eyebrow above any title on the site; the old `Eyebrow` component is deprecated.
- **No section dividers.** Do not separate sections with hairline rules; those read as a document. Separate them with generous whitespace (**section vertical padding ≥ 128px**) and a signature visual.
- **Immersive, headline-dominant hero.** The hero owns the viewport and leads with type, not a code snippet. Its emerald glow is a **background radial-gradient fill** on the hero frame (dark-emerald center fading to near-black edge), **not** a floating / absolute ellipse. Build note: absolute and overlay elements paint *above* content in both Pencil and CSS stacking, so a glow placed as an overlay would sit on top of the headline; it must be a background fill.
- **One signature visual per section.** Each major section is anchored by a single signature visual, not text alone (rails branch diagram, ledger receipt, rebuild stack, lifecycle pipeline, settlement split, and the simulator stage).
- **Spacing frame:** content max-width 1080; desktop artboard 1440 with ~180px side gutters; mobile 390; section vertical padding ≥ 128px.
...
- **Confident, declarative, reliability-signaling.** State capabilities as facts, in the present tense; a subscriptions engine that hedges reads as one that is unsure. Confidence is not hype; it lives inside the truthfulness guardrails below, never outside them.
- **Truthful about the rails, always.** Confident copy never overstates what ships. Card recurring is best-effort charge plus an OTP-to-checkout-link fallback (not silent); direct debit (NIBSS mandate) is the reliable silent rail; outbound webhooks are signed, at-least-once, deduplicated, retried, and replayable (never "exactly-once"); direct debit, payouts, refunds, and escrow are built but live-gated, not proven-live; crypto is a first-class but architecturally distinct rail. State each as it is.
...
- **No em dashes anywhere.** Use a period, a comma, or restructure the sentence. This applies to shipped copy and to these docs going forward.
