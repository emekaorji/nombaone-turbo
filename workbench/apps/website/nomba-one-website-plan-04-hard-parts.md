...
- Frontmatter schema:
  ...
  updated:      date
  ```
- The `/guides` index lists articles grouped by `group`, each card = `title` + `problem`. Featured articles surface on the home page (doc 01 §3.6).
...
Each section is short. The whole piece is a focused read, not an essay. Voice per doc 00 §7.
...
- Voluntary vs involuntary churn — why conflating them corrupts your metrics
...
- Card tokens expire — why blind retries waste your dunning window
...
**4 · How Nomba One handles it.** Every charge attempt carries a stable idempotency reference derived from the subscription and the billing period — the same period always produces the same reference. The scheduler is built to be replayed: a unique constraint on (subscription, period) makes a duplicate invoice structurally impossible, and the reference tells the rail "this is the same attempt, not a new one." Kill the job anywhere and restart it — it resolves to exactly one charge and one ledger entry, every time. The database makes the double-charge impossible; it isn't guarded by hopeful code.
